...
- **Client 刷新合并**：`GitClientController.refresh()` 对同一工作区复用在途刷新（复用键为 `path` + `operationGeneration`，只有同 `generation` 才复用；轮次结束在 `finally` 中释放槽位），composer 控件的 `window` focus 监听因此不会在焦点抖动时叠加多轮 discover/status，也不会让重叠轮次互相判旧。背景见 [troubleshooting](../troubleshooting/windows-desktop-console-flood.md)。
